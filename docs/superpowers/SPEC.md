...
- Provider/model 接入层必须产品化 — 下一阶段应从固定 `openai/anthropic/google` 槽位升级为可新增自定义供应商、可配置 base URL / headers、可绑定自定义模型的接入注册表
...
| 问题 | P级 | 说明 |
|------|-----|------|
| 自定义供应商 / 自定义模型尚未产品化 | P1 | 当前仍以少量内建 provider 槽位为中心，缺少 provider registry、模型目录和面向终端用户的接入配置体验 |
