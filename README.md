...
- `npm run e2e` now also proves that completed threads survive both mocked continuity flows and real production web-process restarts. The suite covers first-run launch, workspace-root settings saves, coordinator workbench rendering, healthy restored-thread continuity, restored-thread retry recovery, full history-gap detection, partial history-drift detection, and dedicated true process-restart proofs for both the main agent route and the coordinator route using real `next start`, SQLite persistence, and cached preset-catalog fallback. The Vitest suite also includes runtime-level proofs for both fresh-instance SQLite restore and route-level restore during preset-catalog outages.
