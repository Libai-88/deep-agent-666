...
- Restored threads that still have local workbench context but no runtime message history are now detected explicitly and routed into a recovery flow, instead of silently looking empty.
...
- `npm run e2e` now covers first-run launch, restored-thread retry recovery, and restored-thread history-gap detection with deterministic AG-UI stream fixtures. The main remaining runtime hardening task is broader real-backend restart/resume validation across all runtime entrances.
